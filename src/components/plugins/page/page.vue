<template>
    ...
    <ul :class="wrapClasses" :style="styles" v-else>
        <span :class="[prefixCls + '-total']" v-if="showTotal">
            <slot>共 {{ virtual_total }} <template v-if="total <= 1">条</template><template v-else>条</template></slot>
        </span>
        <li
            title="上一页"
            :class="prevClasses"
        ...
        <li title="向前5页" v-if="currentPage - 3 > 1" :class="[prefixCls + '-item-jump-prev']" @click="fastPrev"><a><i class="ivu-icon ivu-icon-ios-arrow-left"></i></a></li>
        ...
        <li title="向后5页" v-if="currentPage + 3 < allPages" :class="[prefixCls + '-item-jump-next']" @click="fastNext"><a><i class="ivu-icon ivu-icon-ios-arrow-right"></i></a></li>
        ...
        computed: {
            ...
            nextClasses () {
                return [
                    `${prefixCls}-next`,
                    {
                        [`${prefixCls}-disabled`]: this.currentPage === this.allPages && this.nextDisable
                    }
                ];
            },
            firstPageClasses () {
        ...
        methods: {
            ...
            next () {
                ...
                if (current >= this.allPages){
                    if(this.nextDisable || (this.beforeNextLoad && this.beforeNextLoad(this, current))){
                        return false;
                    }
                }
                this.changePage(current + 1);
