        ...
        <Form :model="form" :label-width="80" :rules="articleRules" ref="form">
            <Row>
                ...
                <Form-item prop="category" label="类型" style="z-index:999999;width: 100%;float: left">
                    <Select :disabled="cannotEdit" v-model="form.category" placeholder="请选择"
            ...
            <Form-item>
                <Row>
                    <Col span="8">
                <UpImgModal img_id="ArticleImgModal" module_name="上传文章图片" onOkText="插入" :onUploadSuccess="update_news_content" :renamePrefix="form.title" v-show="canEdit">
                </UpImgModal>
                </Col>
                <Col span="8">
                <Button @click="onStickNews" >置顶本文</Button>
                </Col>
                <Col span="8">
                <SlideImgModal img_id="SlideImgModal" button_name="轮播本文" module_name="选择轮播展示图片" onOkText="确定" :onUploadSuccess="set_slide_img" :renamePrefix="form.title" ></SlideImgModal>
                </Col>
            </Row>
            </Form-item>
            <Form-item>
            ...
            <Row>
                <Form-item prop="content">
                    <UMeditor ref="UMeditor" :content="form.content"></UMeditor>
                </Form-item>
            </Row>
            <br>
            <br>
...
//                canEdit: G_CheckAdminPermission(17),
                cannotEdit: !G_CheckAdminPermission(17),
                form: {
                    a_id: '',
                    title: '',
                    category: '',
                        ...
                        self.$axios.post(url, self.form).then(result => {
                            if (result && result.data && result.data.code == 200) {
                                if(self.form.action === 'add'){
                                    ...
                                    //从服务器获取a_id
                                    self.form.a_id = result.data.data;
                                }
                                else{
                ...
                this.$axios.get(url)
                    .then(result => {
                        if (result && result.data && result.data.code == 200) {
                            ...
                            console.log('从服务器获取的详情信息',this.initForm);
                            ...
                            console.log('本地表单初始化信息',this.form);
                        }
                    })
            },
            onStickNews(){
                ...
                this.$axios.post(req_news_modify, {action: "update", a_id: this.form.a_id,stick_time: new Date().getTime(),update_keys:['stick_time']})
                            .then(result => {
...
//                return default_keywords.key;
            ...
            update_news_content(img_url){
                console.log('上传文章图片的回调', img_url);
                this.insertHtml('<img src="' + img_url + '" width="600px"/>');
            },
            set_slide_img(img_url){
                console.log('上传轮播slide图片的回调',img_url);
                let self = this;
                if(this.form.action === 'add'){
                    self.$Message.error('请先添加文章~~');
                    return;
                }
                let url = req_set_scrolling_list;
                let postData = {type:'slide',slide_list:[{a_id:self.form.a_id,pic_path:img_url,slide_time:new Date().getTime()}]}
                console.log('submit DATA',postData);
                self.$axios.post(url, postData).then(result => {
                    if (result && result.data && result.data.code == 200) {
                        self.$Message.info('设置轮播成功');
                    } else {
                        self.$Message.error('设置轮播失败');
                    }
                });
            },
            getChangedFields(){
        ...
        components: {
            UMeditor,
            goback,
            "UpImgModal":UpImgModal,
            "SlideImgModal":UpImgModal
        }
    }
</script>
